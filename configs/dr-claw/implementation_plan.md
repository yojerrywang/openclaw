...
> [!NOTE]
> **Inter-Agent Communication**: In Slack, agents can "see" each other. We enable `allowBots: true` for delegation, with a loop guard: bot-authored messages must explicitly target the receiving bot (mention or parent-thread targeting), so untargeted status chatter does not ping-pong forever.
...
- Ensure bot-loop guard behavior is active for untargeted bot chatter.
...
- **No-loop Test**: Post a generic status text and confirm bots do not endlessly bounce untargeted replies back and forth.
